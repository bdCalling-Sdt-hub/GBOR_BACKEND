  ...
  try {
    ...
    if (role === "admin") {
      allNotification = await Notification.find({ role: role })
        ...
        .skip((page - 1) * limit)
        .sort({ createdAt: -1 });
      notViewed = await Notification.countDocuments({
        viewStatus: "false",
        role: role,
      });
...
async function getAllNotification(
  ...
  try {
    ...
    return res.status(500).json({
      status: "Error",
      statusCode: "500",
      message: "Server error in retriving notifications",
    });
  }
}
...
const getNotificationDetails = async (req, res) => {
  ...
  try {
    ...
    var details;
    if (role === "user") {
      details = await UserModel.findById(notification.linkId).populate(
        "creator"
      );
    ...
    if (role === "admin") {
      ...
      io.to("room" + notification.receiverId.toString()).emit(
        `${role}-notification`,
        data
      );
    }
